 ...
 * RETURNS: None.
 */
static VOID AK2_RndShaderLog( CHAR *Stage, CHAR *Text )
{
  FILE *F;

  if ((F = fopen("AK2{SHAD}30.LOG", "a")) != NULL)
  {
    fprintf(F, "%s: %s\n", Stage, Text);
    fclose(F);
  }
} /* End of 'AK2_RndShaderLoadTextFile' function */
 ...
 */
static CHAR * AK2_RndShaderLoadTextFile( CHAR *FileName )
{
  FILE *F;
  CHAR *text;
  INT flen;
 ...
 */
UINT AK2_RndShaderLoad( CHAR *FileNamePrefix )
{
  INT i, res;
  CHAR *txt;
  CHAR *suff[] = {"VERT", "FRAG"};
  ...
  for (i = 0; isok && i < NS; i++)
  {
    ...
    {
      AK2_RndShaderLog(suff[i], "Error create shader");
      isok = FALSE;
      break;
    }
    /* Load text file */
    sprintf(buf, "%s.%s", FileNamePrefix, suff[i]);
    txt = AK2_RndShaderLoadTextFile(buf);
    if (txt == NULL)
    {
      AK2_RndShaderLog(suff[i], "No file");
      isok = FALSE;
      break;
    }
  ...
  return prg;
} /* End of 'AK2_RndShaderLoad' function */
 ...
 * RETURNS: None.
 */
VOID AK2_RndShaderFree( UINT Prg )
{
  UINT i, n, shds[5];
  ...
  glDeleteProgram(Prg);
} /* End of 'AK2_RndShaderFree' function */
