...
BOOL AK2_RndObjLoad( ak2OBJ *Obj, CHAR *FileName )
{
  ...
  AK2_RndLoadMaterials(MtlFile);

  /* Allocate mnemory for primitives */
  if ((Obj->Prims = malloc(sizeof(ak2PRIM) * NumOfPrimitives)) == NULL)
  {
    fclose(F);
    return FALSE;
  }
  ...
  for (p = 0; p < NumOfPrimitives; p++)
  {
    ...
    if ((V = malloc(sizeof(ak2VERTEX) * NumOfV + sizeof(INT) * NumOfI)) == NULL)
    {
      while (p-- > 0)
      {
    ...
    Obj->Prims[p].MtlNo = AK2_RndFindMaterial(Mtl);
    fread(V, sizeof(ak2VERTEX), NumOfV, F);
    ...
    /* Store vertex data */
    glBufferData(GL_ARRAY_BUFFER, sizeof(ak2VERTEX) * NumOfV, V, GL_STATIC_DRAW);
    ...
    glVertexAttribPointer(0, 3, GL_FLOAT, FALSE, sizeof(ak2VERTEX),
                          (VOID *)0); /* position */
    glVertexAttribPointer(1, 2, GL_FLOAT, FALSE, sizeof(ak2VERTEX),
                          (VOID *)sizeof(VEC)); /* texture coordinates */
    glVertexAttribPointer(2, 3, GL_FLOAT, FALSE, sizeof(ak2VERTEX),
                          (VOID *)(sizeof(VEC) + sizeof(VEC2))); /* normal */
    glVertexAttribPointer(3, 4, GL_FLOAT, FALSE, sizeof(ak2VERTEX),
  ...
  fclose(F);
  return TRUE;
} /* End of 'AK2_RndObjLoad' function */
